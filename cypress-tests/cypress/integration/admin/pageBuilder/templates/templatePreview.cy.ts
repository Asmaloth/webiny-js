...
context("Page Builder - Blocks", () => {
    ...
    it("Should be able to create a page and view all existing templates in it", () => {
        ...
        cy.get(".css-1rl9ll7-listStyle .css-5bicyh-listItem").each((item, index) => {
            ...
            cy.contains(
                pageTemplateData1.title || pageTemplateData2.title || pageTemplateData3.title
            ).should("exist");
            cy.contains(
                pageTemplateData1.description ||
                    pageTemplateData2.description ||
                    pageTemplateData3.description
            ).should("exist");
        });
    });
});
